...
| 3 | `call_module` 的**审批按目标模块风险等级**，不一律放行 | ✅ **真机证实**（见 §4.3.7） |
...
| 14 | **模型正确引用函数参数**（`{{vars.<name>}}`） | ✅ **真机证实**（见 §4.3.7） |
...
#### 4.3.7 第五轮真机验证（2026-09-15）—— 审批分级 + 引用语法修复，双双通过

**① 验收项 3：`call_module` 按目标模块风险等级审批 ✅**

**测法**：把自动审批档位设成 `LOW_RISK`，然后依次调用三个**风险等级不同**、
但**都走 `call_module` 同一入口**的模块——唯一变量是目标模块的等级。

| 模块 | 等级 | 期望 | 结果 |
|---|---|---|---|
| `vflow.device.flashlight` | `LOW` | 自动放行，不弹审批 | ✅ |
| `vflow.system.darkmode` | `STANDARD` | 弹审批 | ✅ |
| `vflow.core.shell_command` | `HIGH` | 弹审批 | ✅ |

**判据的强度**：三个模块经的是同一个工具入口，**唯一差别是目标模块的风险等级**，
故这组结果能排除了「恰好没弹」的偶然性。若三个都不弹，则意味着
`call_module` 成了**绕过全部风险评估的后门**（`prepareCallModule` 用
`getRiskLevelForModuleId(moduleId)` 构造 definition 的设计意义即在此）。

**② 验收项 14：函数参数引用语法修复 ✅**

模型不再写裸 `{{user_id}}`，改用 `{{vars.user_id}}`。§4.3.6 的提示词补充生效。

**至此，第二批验收 14 项中 13 项已闭合**，仅剩第 8 项（Anthropic 缓存命中）——
...
### v1.5.9（2026-09-15）—— 第五轮真机验证，两批验收基本收口

**一、验收项 3 通过：`call_module` 按目标模块风险等级审批**（§4.3.7）

档位设 `LOW_RISK`，三个风险等级不同的模块经同一 `call_module` 入口：
`flashlight`(LOW) 自动放行、`darkmode`(STANDARD) 弹审批、`shell_command`(HIGH) 弹审批。
**唯一变量是目标模块等级**，故能排除偶然性。

**二、验收项 14 通过：函数参数引用语法修复生效**

**三、第二批验收 14 项 → 13 项闭合**

仅剩第 8 项（Anthropic 缓存命中）——需先加 `cache_read_input_tokens` 日志，
属独立小任务。另：§6.2 开放问题 6 项中 5 项已解决，仅剩第 4 项（同上）。
