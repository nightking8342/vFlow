...
| **4** | Chat 悬浮窗 | 用户可边看屏幕边对话 | **需求设计已完成 → [`chat-float-window-design.md`](chat-float-window-design.md)**（§4 为调研底稿） |
...
> **➡️ 实施请看 [`chat-float-window-design.md`](chat-float-window-design.md)**（需求设计 v1.0，已定形态=折叠/展开双形态、审批=窗内直接审批、状态=Application 作用域共享 VM）。本节保留为**调研底稿**。
>
